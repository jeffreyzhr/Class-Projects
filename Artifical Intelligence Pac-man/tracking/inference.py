...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for p in all_pos:
        for g in all_pos:
            temp = manhattanDistance(p, g)
            if temp > dist:
                dist = temp
    
    variableDomainsDict[OBS0] = range(dist + MAX_NOISE + 1)
    variableDomainsDict[OBS1] = range(dist + MAX_NOISE + 1)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            ...
            if joinedFactor.unconditionedVariables() != {var}:
                currentFactorsList.append(joinedFactor)
            list_length = len(currentFactorsList)
            for i in range(list_length):
                factor = currentFactorsList[i]
                if var in factor.unconditionedVariables():
                    currentFactorsList[i] = eliminate(factor, var)
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None:
            ...
            probability = busters.getObservationProbability(noisyDistance, manhattanDistance(ghostPosition, pacmanPosition))
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghost_pos in self.allPositions:
            new_belief = self.getObservationProb(observation, gameState.getPacmanPosition(), ghost_pos, self.getJailPosition())
            old_belief = self.beliefs[ghost_pos]
            self.beliefs[ghost_pos] = new_belief * old_belief
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for item in newPosDist.items():
                np, probability = item
                new_dist[np] += self.beliefs[oldPos] * probability
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        particleseach = self.numParticles //len(self.legalPositions)
        count = 0
        for p in self.legalPositions:
            for i in range(particleseach):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        toupdate = DiscreteDistribution()
        
        for p in self.particles:
            prob = self.getObservationProb(observation, gameState.getPacmanPosition(), p, self.getJailPosition())
            toupdate[p] = toupdate[p] + prob
        
        if toupdate.total() == 0:
            self.initializeUniformly(gameState=busters.GameState)
            toupdate = self.getBeliefDistribution()
        
        toupdate.normalize()
        self.particles = [toupdate.sample() for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for item in newPosDist.items():
                np, probability = item
                new_dist[np] += self.getBeliefDistribution()[oldPos] * probability
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        ghosts = list(ghosts)
        random.shuffle(list(ghosts))
        self.particles = [ghosts[i % len(ghosts)] for i in range(self.numParticles)]
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if dist is None:
                    ...
                    prob = prob * self.getObservationProb(dist, gameState.getPacmanPosition(), p[i], jail)
                
            new_dist[p] = new_dist[p] + prob
